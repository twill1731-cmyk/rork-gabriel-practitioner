...
export default function IntakeScreen() {
  ...
  // Track keyboard height manually (like ChatGPT/Claude)
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]}>
      ...
      {(showInput === 'text' || showInput === 'date') && !isComplete && (
        <View style={[styles.inputBar, { paddingBottom: keyboardHeight > 0 ? 8 : Math.max(insets.bottom, 8), marginBottom: keyboardHeight > 0 ? keyboardHeight - insets.bottom : 0 }, IS_TABLET && styles.inputBarTablet]}>
          <TextInput
            ref={inputRef}
...
const styles = StyleSheet.create({
  ...
  header: {
    ...
    borderBottomWidth: 1, borderBottomColor: Colors.border, backgroundColor: Colors.card, gap: 10,
  },
  backBtn: { padding: 4 },
  headerCenter: { flex: 1, flexDirection: 'row', alignItems: 'center', gap: 10 },
  headerIconSmall: { width: 28, height: 28, borderRadius: 14, backgroundColor: Colors.tealLight, justifyContent: 'center', alignItems: 'center' },
  headerIconText: { fontSize: 12, color: Colors.teal },
  headerTitle: { fontSize: 15, fontFamily: Fonts.semiBold, color: Colors.text, letterSpacing: 0.2 },
  headerSubtitle: { fontSize: 10, fontFamily: Fonts.regular, color: Colors.textTertiary },
  progressBar: { height: 2, backgroundColor: Colors.bgTertiary },
  progressFill: { height: 3, backgroundColor: Colors.teal, borderRadius: 1.5 },
  chatArea: { flex: 1 },
  chatContent: { padding: 12, paddingTop: 10, paddingBottom: 16, gap: 10 },
  chatContentTablet: { maxWidth: 680, alignSelf: 'center', width: '100%' },

  // Bubbles
  gabrielBubble: { flexDirection: 'row', alignItems: 'flex-start', gap: 8, maxWidth: '88%' },
  avatar: { width: 28, height: 28, borderRadius: 14, backgroundColor: Colors.tealLight, justifyContent: 'center', alignItems: 'center', marginTop: 2 },
  avatarText: { fontSize: 12, color: Colors.teal },
  gabrielContent: { flex: 1, backgroundColor: Colors.card, borderRadius: 16, borderTopLeftRadius: 4, paddingHorizontal: 14, paddingVertical: 10, borderWidth: 1, borderColor: Colors.cardBorder },
  gabrielText: { fontSize: IS_TABLET ? 16 : 15, fontFamily: Fonts.regular, color: Colors.text, lineHeight: IS_TABLET ? 24 : 21 },
